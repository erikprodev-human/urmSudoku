...
from URM_Core import URMConfig, URM, CastedSparseEmbeddingSignSGD, stablemax_cross_entropy, IGNORE_LABEL_ID
...
def add_input_noise(inputs: torch.Tensor, noise_ratio: float = 0.1, blank_token: int = 1) -> torch.Tensor:
    noisy = inputs.clone()
    noisy[(inputs > blank_token) & (torch.rand_like(inputs, dtype=torch.float32) < noise_ratio)] = blank_token
    return noisy
...
def estimate_pass_at_k(num_samples: int, num_correct: int, k: int) -> float:
    ...
    prob_all_wrong = 1.0
    for i in range(k):
        prob_all_wrong *= (num_samples - num_correct - i) / (num_samples - i)
    return 1.0 - prob_all_wrong

def evaluate_puzzle_pass_at_k(model, batch, config, num_samples: int = 32, noise_ratio: float = 0.1, k: int = 16):
    batch_size, labels, mask = batch["inputs"].shape[0], batch["labels"], batch["labels"] != IGNORE_LABEL_ID
    pass_rates = []
    with torch.no_grad():
        for b in range(min(batch_size, 16)):
            puzzle_labels, puzzle_mask = labels[b], mask[b]
            if puzzle_mask.sum() == 0:
                continue
            inputs_expanded = batch["inputs"][b:b+1].expand(num_samples, -1).clone()
            if noise_ratio > 0:
                inputs_expanded = add_input_noise(inputs_expanded, noise_ratio=noise_ratio)
            sample_batch = {"inputs": inputs_expanded, "labels": batch["labels"][b:b+1].expand(num_samples, -1), "puzzle_identifiers": batch["puzzle_identifiers"][b:b+1].expand(num_samples)}
            carry = model.initial_carry(sample_batch)
            for _ in range(config.loops):
                carry, outputs = model(carry, sample_batch)
            predictions = torch.argmax(outputs["logits"], dim=-1)
            mask_expanded, labels_exp = puzzle_mask.unsqueeze(0).expand(num_samples, -1), puzzle_labels.unsqueeze(0).expand(num_samples, -1)
            num_correct = ((predictions == labels_exp) | ~mask_expanded).all(dim=1).sum().item()
            pass_rates.append(estimate_pass_at_k(num_samples, num_correct, k))
    return np.mean(pass_rates) if pass_rates else 0.0

def evaluate_puzzle_pass_at_1(model, batch, config):
    """Evaluate pass@1 without noise (normal usage)."""
    batch_size, labels, mask = batch["inputs"].shape[0], batch["labels"], batch["labels"] != IGNORE_LABEL_ID
    correct_count = 0
    with torch.no_grad():
        for b in range(min(batch_size, 16)):
            ...
            if puzzle_mask.sum() == 0:
                continue
            single_batch = {"inputs": batch["inputs"][b:b+1], "labels": batch["labels"][b:b+1], "puzzle_identifiers": batch["puzzle_identifiers"][b:b+1]}
            carry = model.initial_carry(single_batch)
            for _ in range(config.loops):
                carry, outputs = model(carry, single_batch)
            prediction = torch.argmax(outputs["logits"][0], dim=-1)
            if ((prediction == puzzle_labels) | ~puzzle_mask).all():
                correct_count += 1
    return correct_count / min(batch_size, 16) if batch_size > 0 else 0.0
...
def evaluate_model(model, config, step, ema=None):
    ...
    viz_batch, viz_frames = {k: v[0:1] for k, v in batch.items()}, []
    carry = model.initial_carry(viz_batch)
    labels_viz, input_ids = viz_batch["labels"][0].cpu().numpy(), viz_batch["inputs"][0].cpu().numpy()
    input_grid, target_grid_decoded = decode_grid(input_ids), decode_grid(labels_viz)
    ...
    viz_frames[0].save(save_path, save_all=True, append_images=viz_frames[1:], duration=500, loop=0)
    pass_at_1 = evaluate_puzzle_pass_at_1(model, batch, config)
    pass_at_16 = evaluate_puzzle_pass_at_k(model, batch, config, num_samples=32, noise_ratio=0.08, k=16)
    model.train()
    if ema is not None:
        ema.restore()
    new_save_path = f"results/step_{step}_pass1_{pass_at_1*100:.1f}.gif"
    os.rename(save_path, new_save_path)
    return pass_at_1, pass_at_16

def save_checkpoint(model, optimizer, adamw_optimizer, puzzle_optimizer, ema, step, config, loss_history=None, accuracy_history=None):
    os.makedirs(config.checkpoint_dir, exist_ok=True)
    # Get the underlying model if compiled
    model_to_save = model._orig_mod if hasattr(model, '_orig_mod') else model
    checkpoint = {"step": step, "model_state_dict": model_to_save.state_dict(), "optimizer_state_dict": optimizer.state_dict(),
                  "loss_history": loss_history or [], "accuracy_history": accuracy_history or []}
    ...
    torch.save(checkpoint, os.path.join(config.checkpoint_dir, f"checkpoint_step_{step}.pt"))
...
def load_latest_checkpoint(config):
    ...
    for checkpoint_path in sorted(checkpoint_files, key=get_step, reverse=True):
        ...
        try:
            ...
            # Remove _orig_mod. prefix if present (from compiled model checkpoints)
...
def save_training_graphs(loss_history, accuracy_history, step, config):
    ...
    if accuracy_history:
        ...
        pass1_accs = [h[1] * 100 for h in accuracy_history]
        pass16_accs = [h[2] * 100 for h in accuracy_history]
        ax2.plot(steps_acc, pass1_accs, 'b-', linewidth=2, marker='o', markersize=4, label='pass@1 (no noise)')
        ax2.plot(steps_acc, pass16_accs, 'g-', linewidth=2, marker='s', markersize=4, label='pass@16 (8% noise)')
        ax2.set_xlabel('Step'); ax2.set_ylabel('Accuracy (%)'); ax2.set_title('Accuracy over Time')
...
def get_parameter_groups(model, config):
    """Split parameters into Muon-compatible (2D) and AdamW (others) groups."""
    muon_params = []
    adamw_params = []
    
    # Get puzzle embedding local weights to exclude
    ...
    for name, param in model.named_parameters():
        ...
        if puzzle_local_weights is not None and param is puzzle_local_weights:
            continue  # Skip puzzle embedding local weights (handled separately)
        
        # Muon is for 2D parameters (weight matrices in hidden layers)
        # AdamW is for bias, embeddings, and other non-2D parameters
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train URM Sudoku Model")
...
def main():
    ...
    print(f"Optimizer: Muon (momentum={config.muon_momentum}, ns_steps={config.muon_ns_steps}) + AdamW")
    print("=" * 60)
    ...
    config.vocab_size, config.seq_len, config.num_puzzle_identifiers = dataset.metadata.vocab_size, dataset.metadata.seq_len, dataset.metadata.num_puzzle_identifiers

    print("Creating model...")
    model = URM(config).to(config.device)
    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")

    # Load checkpoint BEFORE compiling and creating optimizers
    ...
    # Create EMA after loading checkpoint (so it tracks the loaded weights)
    ...
    # Split parameters for Muon (2D weights) and AdamW (embeddings, bias, etc.)
    # Need to get params from the underlying model if compiled
    ...
    # AdamW optimizer for embeddings, bias, and other non-2D parameters
    ...
    # Puzzle embedding optimizer (sparse)
    ...
    accuracy_history_raw = checkpoint.get("accuracy_history", []) if checkpoint else []
    accuracy_history = []
    for e in accuracy_history_raw:
        if len(e) == 3:
            accuracy_history.append(e)
        elif len(e) == 2:
            accuracy_history.append((e[0], e[1], e[1]))
    ...
    try:
        while step < config.epochs:
            ...
            with autocast_ctx:
                ...
                labels, mask = carry.current_data["labels"], carry.current_data["labels"] != IGNORE_LABEL_ID
            ...
            if accumulation_counter >= config.gradient_accumulation_steps:
                ...
                if step % config.eval_interval == 0:
                    ...
                    pass_at_1, pass_at_16 = evaluate_model(model, config, step, ema=ema)
                    accuracy_history.append((step, pass_at_1, pass_at_16))
                    tqdm.write(f"Step {step}: Loss={current_loss:.4f}, pass@1={pass_at_1*100:.2f}%, pass@16={pass_at_16*100:.2f}%, LR={lr:.2e}")

                if step % config.checkpoint_interval == 0:
                    save_checkpoint(model, optimizer, adamw_optimizer, puzzle_optimizer, ema, step, config, loss_history, accuracy_history)
