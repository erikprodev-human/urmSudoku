...
def evaluate_puzzle_pass_at_k(model, batch, config):
    """Evaluate pass@K where K = config.loops (max reasoning steps)."""
    ...
    with torch.no_grad():
        for b in range(min(batch_size, 64)):
            ...
            if ((prediction == puzzle_labels) | ~puzzle_mask).all():
                correct_count += 1
    return correct_count / min(batch_size, 64) if batch_size > 0 else 0.0
...
def evaluate_model(model, config, step, ema=None):
    ...
    if ema is not None:
        ema.restore()
    new_save_path = f"results/step_{step}_pass{config.loops}_{pass_at_k*100:.1f}.gif"
...
def save_training_graphs(loss_history, accuracy_history, step, config):
    ...
    if accuracy_history:
        ...
        ax2.plot(steps_acc, pass_k_accs, 'b-', linewidth=2, marker='o', markersize=4, label=f'pass@{config.loops} ({config.loops} reasoning steps)')
        ax2.set_xlabel('Step'); ax2.set_ylabel('Accuracy (%)'); ax2.set_title(f'Accuracy over Time (pass@{config.loops})')
        ...
        while step < config.epochs:
            ...
            if accumulation_counter >= config.gradient_accumulation_steps:
                ...
                if step % config.eval_interval == 0:
                    ...
                    tqdm.write(f"Step {step}: Loss={current_loss:.4f}, pass@{config.loops}={pass_at_k*100:.2f}%, LR={lr:.2e}")
